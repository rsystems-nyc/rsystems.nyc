...
Get a footprint in the big clouds. Google and Microsoft are the essential two; Amazon matters less for most organizations day to day, though we like to set up an AWS footprint as well because there are things Amazon does better than either, and using best-of-breed for each has real value. But there is no company — short of being Google or being Microsoft — that shouldn't have both a Google and a Microsoft [tenant](/resources/glossary/tenant) set up.
